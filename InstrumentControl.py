...
class osc(VISADevice):
    ...
    Q = []
    Det = []
    data = np.zeros((2,1),dtype=np.float)
    data1 = np.zeros((2,1),dtype=np.float)
    data2 = np.zeros((2,1),dtype=np.float)
    data3 = np.zeros((2,1),dtype=np.float)
...
class lockin(VISADevice):
    ...
    lockdata = np.zeros((2,1),dtype=np.float)
    datanumber = np.zeros((2,1),dtype=np.float)
    ...
    def getdataASCIIfp(self,args=[0,16383]):   # args = [startbin,endbin], which are the bins of data to request.
        ...
        self.lockdata = np.zeros((2,args[1]-args[0]),dtype=np.float)
...
def scanmag(oscilloscope,magnet,keithley,timestep,endfield,ioff,qoff,savefile):
    ...
    plott = 0
    #lock.lockdata = np.zeros((2,numberofsteps+5),dtype=np.float)
    starttime = time.time()
